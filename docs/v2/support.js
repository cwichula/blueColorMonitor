/* Monitor Światła v2 — warstwa wsparcia (dobrowolna darowizna na Buy Me a Coffee).
 *
 ...
 * and no advertising layer: every one of the seven metrics, the whole history,
 * all seven tools and the offline mode work for everybody, always, with no
 * condition attached. Nothing in this file can gate a feature, because this
 * file exposes nothing a feature could ask.
 *
 ...
 *
 * No network of any kind. The Buy Me a Coffee widget would fetch a script from
 * its servers on every launch, which would break both the offline mode and the
 * promise this application makes on every screen; the cup icon is therefore
 * drawn locally, in styles.css, exactly like every other icon here.
 *
...
(function (global) {
  ...
  /* ------------------------------------------------------------------
     ...
     ------------------------------------------------------------------
     Two conditions, both narrow on purpose: the scheme must be https: and the
     host must be buymeacoffee.com (or its www. form), because Buy Me a Coffee
     is the only place this application ever sends anybody. A typo in the
     scheme, an http: address, a pasted `javascript:` URL and a link to any
     other service all resolve to "no address configured", which is a state the
     screen already renders correctly. There is no third case.
     ------------------------------------------------------------------ */

  var SUPPORT_HOSTS = ['buymeacoffee.com', 'www.buymeacoffee.com'];

  function allowedHost(host) {
    var name = String(host || '').toLowerCase();
    for (var i = 0; i < SUPPORT_HOSTS.length; i += 1) {
      if (SUPPORT_HOSTS[i] === name) return true;
    }
    return false;
  }
  ...
  function supportUrl() {
    ...
    try {
      ...
      return allowedHost(u.hostname) ? u.href : '';
    } catch (e) {
      return '';
    }
  }
  ...
  /* ------------------------------------------------------------------
     Clean-up after older builds
     ------------------------------------------------------------------
     A device that ran an earlier build still carries those keys. Nothing reads
     them any more, so they are dead weight and they go on the first launch of
     this version. The list is explicit: no prefix sweep, because ms2.history,
     ms2.settings, ms2.thresholds and ms2.profiles share that prefix and must
     survive.
     ------------------------------------------------------------------ */
